...
def remove_friend():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        results = executor.map(lambda token: remove(token, hex_encrypted_data), tokens)

    success_count = sum(1 for result in results if result)

    return jsonify({"message": f"SUCCESSFULLY REMOVED {success_count} FRIENDS"}), 200
